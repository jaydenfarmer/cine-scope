...
import { Media, ApiResponse, Trailer, MediaType } from '../shared/interfaces/media.interface';
import { API_CONFIG, IMAGE_SIZES, BACKDROP_SIZES } from '../shared/constants/api.constants';
...
export class TmdbService {
  ...
  getLatestTrailers(category: 'popular' | 'streaming' | 'on_tv' | 'for_rent' | 'in_theaters') {
    switch (category) {
      case 'popular':
        return this.http.get<any>(`${this.baseUrl}/movie/popular?api_key=${this.apiKey}&language=en-US&page=1`);
      case 'streaming':
        return this.http.get<any>(`${this.baseUrl}/trending/movie/week?api_key=${this.apiKey}&language=en-US`);
      case 'on_tv':
        return this.http.get<any>(`${this.baseUrl}/tv/airing_today?api_key=${this.apiKey}&language=en-US&page=1`);
      case 'for_rent':
        return this.http.get<any>(`${this.baseUrl}/movie/upcoming?api_key=${this.apiKey}&language=en-US&page=1`);
      case 'in_theaters':
        return this.http.get<any>(`${this.baseUrl}/movie/now_playing?api_key=${this.apiKey}&language=en-US&page=1`);
      default:
        return this.http.get<any>(`${this.baseUrl}/movie/popular?api_key=${this.apiKey}&language=en-US&page=1`);
    }
  }
  ...
  getUpcomingTrailers(type: 'movie' | 'tv' = 'movie') {
    ...
    return this.http.get<any>(`${this.baseUrl}/${endpoint}?api_key=${this.apiKey}&language=en-US&page=1`);
  }
  ...
  searchPeople(query: string) {
    return this.http.get<any>(`${this.baseUrl}/search/person?api_key=${this.apiKey}&query=${encodeURIComponent(query)}`);
  }
  ...
  getTopRatedTvShows(page: number = 1): Observable<any> {
    return this.http.get(`${this.baseUrl}/tv/top_rated?api_key=${this.apiKey}&page=${page}`);
  }
  ...
  getPopularPeople(page: number = 1): Observable<any> {
    return this.http.get(`${this.baseUrl}/person/popular?api_key=${this.apiKey}&page=${page}`);
  }
  ...
  getMovieProviders(): Observable<any> {
    return this.http.get(`${this.baseUrl}/watch/providers/movie?api_key=${this.apiKey}&language=en-US`);
  }  
  ...
  if (filters.certifications?.length) {
    ...
    if (filters.language && filters.language !== '') params.push(`with_original_language=${filters.language}`);
    ...
    if (isUpcomingPage && mediaType === 'movie') {
      ...
      if (filters.releaseFrom) params.push(`primary_release_date.gte=${filters.releaseFrom}`);
      if (filters.releaseTo) params.push(`primary_release_date.lte=${filters.releaseTo}`);
    }
